...
class GeneticAlgorithm_AD(RandomSearch):
    ...
    def search(self, n_iterations, min_iterations, max_pressure, report=False, log=False, plot=True):
        ...
        for iteration in range(n_iterations):
            ...
            self.pressure += -self.geno_shift * 0.02 + 0.0000003 * (np.power(self.iteration, 2))

            if self.pressure > max_pressure:
                self.pressure = max_pressure
            self.selection = uls.parametrized_tournament_selection_ver3(self.pressure )
            offsprings = []
            ...
            while len(offsprings) < len(self.population):
                ...
                if self._random_state.uniform() < self.p_c:
                    off1, off2 = self._crossover(p1, p2, p3, p4)
                    self.problem_instance.evaluate(off1)
                    self.problem_instance.evaluate(off2)
                    if self._random_state.uniform() < 0.8:
                        all = [p1, p2, off1, off2]
            ...
            self.p_m -= self.geno_shift*0.1*self.p_m
            #(0.005 -  0.005 *(1-self.param))           #-(-((self.param*100-100)/100)*self.p_m_max)
            # print(self.p_c)
            # print(self.p_m)
            ...
            self.fitness_list.append(elite.fitness)
    ...
    def _crossover(self, p1, p2, p3, p4):
        off1, off2 = self.crossover(p1.representation, p2.representation,p3.representation, p4.representation, self._random_state)
        off1, off2 = Solution(off1), Solution(off2)
        self.problem_instance.evaluate(off1)
        self.problem_instance.evaluate(off2)
        return off1, off2
