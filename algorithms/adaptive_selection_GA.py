...
class GeneticAlgorithm(RandomSearch):
    def __init__(self, problem_instance, random_state, population_size,
                pressure, update_rate, crossover, p_c, mutation, p_m, index):
    ...
    def search(self, n_iterations, min_iterations, max_pressure,report=False, log=False,plot=True):
        ...
        for iteration in range(n_iterations):
            ...
            self.selection = uls.parametrized_tournament_selection(self.pressure,)
            offsprings = []
            ...
            for i in offsprings:
                if i.fitness == 0:
                    list_invalid.append(1)
            self.invalid_percentage.append(len(list_invalid)/len(offsprings))
            ...
            if log:
                log_event = [iteration, elite.fitness, elite.validation_fitness if hasattr(off2, 'validation_fitness') else None,
                             ...
                             self.mutation.__name__, self._random_state, None, self.p_m, self.pressure, self.update_rate, self._phenotypic_diversity_shift(offsprings), self._gen_pop_diversity(self.population)], self._std(self.population)
