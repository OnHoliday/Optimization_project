...
from algorithms.GA_adaptive import GeneticAlgorithm_AD
...
#
# # combinations = [[0.9,0.95,1,0.2,1.03,20,0.40,0.5,125,40],[0.95,0.95,1,0.25,1.03,20,0.40,0.5,125,40]]
#
# lista = []
# df = pd.DataFrame()
#
...
#     ga = GeneticAlgorithm(ann_op_i, random_state, ps, pressure, update_rate, uls.two_point_crossover, p_c,
#                           uls.parametrized_ball_mutation(radius), p_m, 'test')
#     ga.initialize(plot=True)
#     ga.search(n_gen, min_iterations, max_pressure, True, False)
#
...
#
#     lista = [p_m, p_c, seed, pressure, update_rate, min_iterations, max_pressure, ga.best_solution.fitness,
#              unseen_fitness]
#     return lista
#
#
# a = Parallel(n_jobs=-1)(
#     delayed(eval)(combination, combinations.index(combination), len(combinations)) for combination in combinations)
#
# df = pd.DataFrame(a,
#                   columns=['p_m', 'p_c', 'seed', 'pressure', 'update_rate', 'min_iterations', 'max_pressure', 'fitness',
#                            'unseen fitness'])
# df.to_csv("Adaptive_tournament_0512_seed4"
...
ga = GeneticAlgorithm_AD(ann_op_i, random_state, ps, pressure, update_rate,
                      uls.two_point_crossover, p_c, uls.parametrized_ball_mutation(radius), p_m, 'test')
ga.initialize(plot=True)
ga.search(n_gen, min_iterations, max_pressure, True, True)
