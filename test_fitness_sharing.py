...
from algorithms.GA_with_plotting_fitshare import GeneticAlgorithm
...
pressure = .25
prob = 0.35
ga = GeneticAlgorithm(ann_op_i, random_state, ps, uls.parametrized_tournament_selection_fitscale(pressure),
                      uls.uniform_crossover, p_c, uls.parametrized_ball_mutation(radius), p_m,'ga')
ga.initialize(plot=True)
ga.search(n_gen, True, True,plot=True)
...
if make_plots:
    ...
    f.subtitle('Testing classifier on unseen data')
    plt.show()
